...
import { Trash2, Plus, Minus, XCircle, CreditCard } from "lucide-react";
...
export function CartSheet({ isOpen, onOpenChange, restaurantId, restaurantWhatsApp, allowOnlinePayment }: CartSheetProps) {
                ...
                <div className="w-full space-y-2">
                  ...
                  {allowOnlinePayment ? (
                    <div className="grid grid-cols-2 gap-2">
                      <Button onClick={() => setIsCheckout(true)} className="w-full" variant="outline" disabled={cartItems.length === 0}>
                        Pedir via WhatsApp
                      </Button>
                      <Button onClick={() => setIsPaymentDialogOpen(true)} className="w-full custom-primary-bg" disabled={cartItems.length === 0}>
